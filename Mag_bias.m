...
for ind_sim = 1:ite_num
    ...
    for ind_data = 1:data_leng
        ...
        R_g2b = euler2R_g2b(limit_pi(euler));                              % get rotation matrix from body to global
    ...
    Q = blkdiag(I3*sigma_g,I3*sigma_g*0.001);                     
    R = I3*sigma_m;
    P_0 = I6*1;
    P_plus = P_0;
    H = [I3, I3];                       % measurement
    for ind_step = 1:data_leng
        omega = data(ind_step, ind_gyro);
        y     = data(ind_step, ind_mag);
        % compute discrete system dynamics
        [Phi, Qd] = compute_system(omega, Q, dt);
        % update
